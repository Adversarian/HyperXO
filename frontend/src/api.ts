...
function connectRoomTauri(roomId: string): WebSocket {
  ...
  const ws: any = {
    ...
    set onmessage(fn: ((ev: any) => void) | null) {
      (ws as any)._onmessage = fn;
    },
    get onmessage() {
      return (ws as any)._onmessage ?? null;
    },
    set onerror(fn: ((ev: any) => void) | null) {
      (ws as any)._onerror = fn;
    },
    get onerror() {
      return (ws as any)._onerror ?? null;
    },
    set onclose(fn: ((ev: any) => void) | null) {
      (ws as any)._onclose = fn;
    },
    get onclose() {
      return (ws as any)._onclose ?? null;
    },
  };
  ...
  import('@tauri-apps/plugin-websocket').then(({ default: TauriWebSocket }) => {
    TauriWebSocket.connect(url).then((tauriWs: any) => {
